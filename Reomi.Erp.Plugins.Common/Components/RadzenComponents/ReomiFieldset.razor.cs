...
{
    [Parameter] public PcSection.PcSectionOptions FieldOptions { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (Node != null)
        {
            // ChildContent = CreateRenderFragment;
        }
    ...
    private static RenderFragment RenderNode(string componentName) => builder =>
    {
        Console.WriteLine(componentName);
        var helperType = Type.GetType(componentName);
        var t = Type.GetType($"Reomi.Erp.Plugins.Common.Components.RadzenComponents.Radzen{helperType?.Name}");
        if (t == null)
            return;
