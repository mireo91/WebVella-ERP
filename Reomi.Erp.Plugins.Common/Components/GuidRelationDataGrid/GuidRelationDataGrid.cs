...
public class GuidRelationDataGrid : PcFieldBase
{
    protected ErpRequestContext ErpRequestContext { get; set; }

    // //
    public GuidRelationDataGrid([FromServices] ErpRequestContext coreReqCtx) 
    {
        ErpRequestContext = coreReqCtx;
    }

    public class DropDownDataGridOptions : PcFieldBaseOptions
    {
        public static DropDownDataGridOptions CopyFromBaseOptions(PcFieldBaseOptions input)
        {
            return new DropDownDataGridOptions
            {
        ...
        try
        {
            ...
            if (context.Node == null)
            {
                return await Task.FromResult<IViewComponentResult>(Content(
                    "Error: The node Id is required to be set as query parameter 'nid', when requesting this component"));
            }
            ...
            var options = DropDownDataGridOptions.CopyFromBaseOptions(baseOptions);
            if (context.Options != null)
            {
                options = JsonConvert.DeserializeObject<DropDownDataGridOptions>(context.Options.ToString());
    ...
    // 		if (context.Options != null)
    // 		{
    // 			options = JsonConvert.DeserializeObject<PcFieldText.PcFieldTextOptions>(context.Options.ToString());
    // 			if (context.Mode != ComponentMode.Options)
    // 			{
    // 				if (options.MaxLength == null)
    // 					options.MaxLength = baseOptions.MaxLength;
